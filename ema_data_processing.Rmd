...
base_path  <- "/users/eu/documents/github/ema-brain/data/raw/" 
...
ema_flags_master <- master_merged %>%
  ...
  mutate(
    ...
    # prompt-level metrics for flags
    ...
    # 2. event flags (3 items)
    flag_event_speed = ifelse(!is.na(dur_event) & dur_event <= NUM_EVENT_ITEMS, 1L, 0L),
    ...
    flag_missing_mood  = ifelse(!is.na(event_mean) & is.na(mood_mean), 1L, 0L),
    flag_missing_event = ifelse(!is.na(mood_mean) & is.na(event_mean), 1L, 0L),
  ...
  select(
    ...
    # event items 
    event_significant, event_stress_raw, event_importance_raw, event_control_raw,
    # sleep items
    sleep_quality, sleep_hours
  )
...
write.csv(ema_flags_master, "data/clean/ema_flags_master.csv", row.names = FALSE)
```
